...
class MessagesControllerTests(ControllerTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        super(MessagesControllerTests, cls).setUpClass()
        cls.controller = cls.api_client.messages
    ...
    Submit one or more (up to 100 per request) SMS or text to voice messages for delivery.
         ...
         - ```delivery_report``` Delivery reports can be requested with each message. If delivery reports are 
          requested, a webhook will be submitted to the ```callback_url``` property specified for the message
          (or to the webhooks) specified for the account every time the status of the message changes as it is
          processed. The current status of the message can also be retrieved via the Delivery Reports endpoint of the
          Messages API. Delivery reports are optional and by default will not be requested.
         - ```destination_number``` The destination number the message should be delivered to. This should be specified
          in E.164 international format. For information on E.164, please refer to http://en.wikipedia.org/wiki/E.164.
         ...
         - ```source_number_type``` If a source number is specified, the type of source number may also be
          ...
          formatted number, available options are ```INTERNATIONAL```, ```ALPHANUMERIC``` or ```SHORTCODE```. Specifying
        a source number type is only valid when the ```source_number``` parameter is specified and is optional.
         ...
         *Note: when sending multiple messages in a request, all messages must be valid for the request to be 
         successful. If any messages in the request are invalid, no messages will be sent.*
    """
    def test_send_messages_1(self):
        ...
        self.assertEqual(body.messages[0]['callback_url'], result.messages[0]['callback_url'],
                         'Callback URL should match exactly (string literal match)')
        self.assertEqual(body.messages[0]['content'], result.messages[0]['content'],
                         'Content should match exactly (string literal match)')
        self.assertEqual(body.messages[0]['format'], result.messages[0]['format'],
                         'Format should match exactly (string literal match)')
        self.assertEqual(body.messages[0]['delivery_report'], result.messages[0]['delivery_report'],
                         'Delivery Report should match exactly (boolean match)')
        self.assertEqual(body.messages[0]['destination_number'], result.messages[0]['destination_number'],
        ...
        self.assertEqual(body.messages[1]['callback_url'], result.messages[1]['callback_url'],
                         'Callback URL should match exactly (string literal match)')
        self.assertEqual(body.messages[1]['content'], result.messages[1]['content'],
                         'Content should match exactly (string literal match)')
        self.assertEqual(body.messages[1]['format'], result.messages[1]['format'],
                         'Format should match exactly (string literal match)')
        self.assertEqual(body.messages[1]['delivery_report'], result.messages[1]['delivery_report'],
                         'Delivery Report should match exactly (boolean match)')
        self.assertEqual(body.messages[1]['destination_number'], result.messages[1]['destination_number'],
                         'Destination Number should match exactly (string literal match)')
